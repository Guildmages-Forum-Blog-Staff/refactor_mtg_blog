...
// node-ical parses VALUE=DATE all-day events to LOCAL midnight, and DTEND is
// exclusive (the day AFTER the last day). These tests construct dates the same
// way (local midnight via `new Date(y, m, d)`), so they are timezone-independent.
...
describe('formatEventDate', () => {
  ...
  it('keeps the month on the end date when the range crosses a month', () => {
    ...
    const start = new Date(2026, 10, 29);
    const end = new Date(2026, 11, 2);
