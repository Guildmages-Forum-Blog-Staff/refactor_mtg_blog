...
// Handle curly double-quotes injected by remark-smartypants
const CURLY_L = String.fromCharCode(0x201c);
const CURLY_R = String.fromCharCode(0x201d);

// No $ at end — open tag may be followed by content in the same text node
const OPEN_RE = /^\{%\s*notel(?:\s+(default|red|blue|green|yellow|purple|orange|cyan))?\s+(.+?)\s*%\}/;
...
export function remarkNotel() {
  return (tree: Root) => {
    ...
    while (i < children.length) {
      ...
      if (!para.children.length || para.children[0].type !== 'text') { i++; continue; }
      ...
      // Search for close tag within this paragraph's text nodes (handles no-blank-line case)
      let closeChildIdx = -1;
      let closeIndex = -1;
      ...
      for (let k = 0; k < para.children.length; k++) {
        ...
        const searchText = k === 0
          ? (child as Text).value.slice(openTagLen)
          : (child as Text).value;
      ...
      if (closeChildIdx !== -1) {
        // Close tag found in same paragraph — extract inner children
        const innerChildren: Paragraph['children'] = [];

        if (closeChildIdx === 0) {
          const innerText = firstText.slice(openTagLen, closeIndex).replace(/^\n/, '').replace(/\n$/, '');
        ...
        if (innerChildren.length > 0) {
          replacements.push({ type: 'paragraph', children: innerChildren } as Paragraph);
        }
      ...
      // Close tag not in same paragraph — search subsequent paragraph nodes
      let j = i + 1;
      while (j < children.length) {
        const candidate = children[j];
        if (candidate.type === 'paragraph') {
          const cPara = candidate as Paragraph;
          const cText = cPara.children.map(c => c.type === 'text' ? (c as Text).value : '').join('');
